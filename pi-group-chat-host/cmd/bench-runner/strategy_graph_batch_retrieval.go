...
var (
	graphBatchSelectionBlockRE = regexp.MustCompile(`(?is)SKILL_SELECTION\s+skill_id:\s*(\S+)(?:\s+sha256:\s*(\S+))?(?:\s+trigger:\s*(.+?))?(?:\nSKILL_SELECTION|\z)`)
	graphBatchNoSkillRE        = regexp.MustCompile(`(?i)\bNO_SKILL(?:_APPLICABLE)?\b`)
	graphBatchSkillURIRE       = regexp.MustCompile(`skill://[^\s]+`)
)
...
func parseGraphBatchSkillSelections(reply string, ledger *pinnedLedger) []graphBatchSkillSelection {
	selections, _ := parseGraphBatchNominations(reply, ledger)
	return selections
}

func parseGraphBatchNominations(reply string, ledger *pinnedLedger) ([]graphBatchSkillSelection, []string) {
	...
	for _, match := range matches {
		...
		sum := strings.TrimSpace(match[2])
		trigger := strings.TrimSpace(match[3])
		raw := strings.TrimSpace(ref + " " + sum)
		if ledger != nil {
			skill, ok, leftover := ledger.resolveNomination(ref, sum)
			if !ok {
				if leftover != "" {
					unresolved = append(unresolved, leftover)
				} else if raw != "" {
...
func parseGraphBatchSelectionFallback(reply string, ledger *pinnedLedger) ([]graphBatchSkillSelection, []string) {
	if ledger == nil {
		return nil, nil
	}
	seen := map[string]bool{}
	var out []graphBatchSkillSelection
	var unresolved []string
	tokens := graphBatchSkillURIRE.FindAllString(reply, -1)
	tokens = append(tokens, pin0917EntryRE.FindAllString(reply, -1)...)
	if len(tokens) == 0 {
		return nil, nil
	}
	for _, token := range tokens {
		...
		skill, ok, leftover := ledger.resolveNomination(token)
		if !ok {
			if leftover != "" {
				unresolved = append(unresolved, leftover)
			}
...
func resolvePinnedSelections(selections []graphBatchSkillSelection, ledger *pinnedLedger) []pinnedSkill {
	...
	for _, selection := range selections {
		skill, ok, _ := ledger.resolveNomination(selection.SkillReference, selection.SHA256)
