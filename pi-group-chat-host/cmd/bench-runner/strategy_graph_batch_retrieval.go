...
// selects structured skill references. Host then directs those skills to
// the upcoming task agent. Legacy warm-skill-batch keeps the old prose-note
// prompt byte-for-byte.
const graphBatchRetrievalPromptHead = "Skill retrieval turn. For this one turn you act as this room's diagnosis agent. A test task is about to start; its prompt is quoted at the end of this message. The frozen skill ledger is available through the skills_list tool: chunk 0 is a compact index — one line per skill with its entry number, name, trigger, sha256 prefix, and the chunk holding its full text. Start from the index, then read only the detail chunks whose triggers plausibly match this task; every response header states total_chunks.\n\nYour job is selection only. Do not write a prose note and do not solve the task. The skills you select will be delivered by the Host as a directed offer to the upcoming task agent.\n\n1. Decide which ledger skills genuinely apply: the trigger must match the task's shape. Select at most 3.\n2. Publish via room_send one message that is exactly one of:\nNO_SKILL\n\nor one or more blocks in this shape (no other prose):\nSKILL_SELECTION\nskill_id: skill://evaluation/<id>@<revision>\nsha256: <64 lowercase hex>\ntrigger: <the skill's trigger, copied verbatim>\n\nQuote skill_id and sha256 exactly as the ledger states them. Solving the quoted task belongs to the task agent's upcoming turn, not to this retrieval turn.\n\n--- upcoming test task ---\n"
